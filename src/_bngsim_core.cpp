...
PYBIND11_MODULE(_bngsim_core, m) {
    ...
    py::class_<bngsim::NetworkModel>(m, "NetworkModel")
        ...
        .def(
            ...
            "not one — a conjunction, a negation, a comparison one of whose sides "
            "is itself a comparison (its residual would be a difference of two "
            "booleans, a step with no gradient), or a comparison that reads no "
            "live model state. The residual identifies the CROSSING rather than "
            "its spelling, so `X<1`, `X<=1` and `X==1` all return the same string "
            "and callers dedupe on it. An equality is admitted here and refused "
            "by the event-trigger path, which needs a rising edge rather than a "
            "surface (issue #381). This is the one authority: the codegen gate "
