...
- `bngsim/third_party/nfsim/VENDOR.json` currently records source ref
  `bngsim/vendor` at commit `a613bb7cc0ff9287f4f7d23fee32a64c618e766d`. That
...
- The current vendored source is based on upstream `origin/master` commit
  `5962ea9ad4248522f6170a61367b1d7242e2824b`.
- Vendoring metadata should record canonical upstream remote
  `https://github.com/RuleWorld/nfsim.git`.
- One carry is a candidate for upstreaming rather than a permanent local
  divergence: `bngsim/carry-total-rate-skips-symmetry-factor` (GH #426) fixes a
  regression that RuleWorld/nfsim `#89` carried into upstream `master`, so it
  belongs in an upstream PR and should be dropped from this queue once it lands
  there. Everything else is a required local carry.
- There are no other active upstream-PR carries. The earlier local carries behind
  RuleWorld/nfsim PRs `#69`, `#70`, `#72`, `#73`, `#74`, `#80`, `#81`, `#82`,
  and `#89` are now in upstream `origin/master` and should stay out of the
  local carry queue.
- RuleWorld/nfsim `#89` merged on 2026-08-18 and took four carries upstream at
  ...
  upstream: it applies the factor to TotalRate rules, which state their whole
  propensity outright and so have no match count to correct. See the
  `bngsim/carry-total-rate-skips-symmetry-factor` entry above and GH #426.
...
- zero-base-rate reactions kept so a post-init `set_param` can activate them
  (GH #44)
- the reaction center symmetry factor skipped under `TotalRate`, which states
  the whole propensity of a rule and therefore has no match count to correct
  (GH #426) — upstream-bound, see the baseline note above
...
every rate law, and pure-context per-complex counting).
