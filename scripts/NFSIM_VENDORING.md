...
- `bngsim/third_party/nfsim/VENDOR.json` currently records source ref
  `bngsim/vendor` at commit `6688167a52acf9a5fa79dc45233d22d7cc969719`. That
...
- zero-base-rate reactions kept so a post-init `set_param` can activate them
  (GH #44)
- reaction center symmetry factor applied on every rate law, not just `Ele`
  (GH #195; candidate to push upstream)
...
15. zero-base-rate reactions kept for post-init `set_param`
16. reaction center symmetry factor on every rate law
