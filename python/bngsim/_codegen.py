...
# Portable noinline attribute, emitted once per translation unit (guarded so the
# RHS + sensitivity sources concatenated into one .so don't redefine it).
_NOINLINE_MACRO_LINES = (
...
def generate_rhs_c(net_path: str) -> str:
    ...
    if chunk:
        for ln in _NOINLINE_MACRO_LINES:
            _emit(ln)
        _emit("")
        for ln in (
            *rxn_block_protos,
    ...
    # ── RHS function ──────────────────────────────────────────────────
    _emit("int bngsim_codegen_rhs(double t, double* y, double* ydot, void* user_data) {")
...
def _emit_sens_rhs_body(
    ...
    _emit("")
    _emit("int bngsim_codegen_sens_rhs(int Ns, double t,")
...
def generate_jacobian_from_model(model) -> str | None:
    ...
    if is_sparse:
        ...
        _emit(
            "int bngsim_codegen_jac_sparse(double t, double* y, double* jac_data, "
            "void* user_data) {"
        )
        _emit("    CodegenUserData* data = (CodegenUserData*)user_data;")
        _emit("    double* p = data->param_values;")
        ...
        _emit("   CodegenUserData typedef and the N_SPECIES/N_OBS/N_FUNC macros. */")
        _emit("int bngsim_codegen_jac(double t, double* y, double* jac, void* user_data) {")
        _emit("    CodegenUserData* data = (CodegenUserData*)user_data;")
        _emit("    double* p = data->param_values;")
...
def generate_outputs_from_model(model) -> str | None:
    ...
    _emit("int bngsim_codegen_outputs(double t, double* y, double* obs_out,")
...
def generate_output_sens_from_model(model) -> str | None:
    ...
    _emit("int bngsim_codegen_output_sens(double t, const double* y, const double* p,")
