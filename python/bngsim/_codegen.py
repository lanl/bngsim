...
# v29: lanl/bngsim #456 — bngsim_mratio answers where it used to return NaN. It
# now falls back to an asymptotic expansion for arguments the continued fraction
# is refused, and the trust test itself gained a bound on positive z that removes
...
_SPECIAL_FUNCTION_C_LINES = (
    ...
    "   there is a change here. The parity bookkeeping is spelled `odd = 1 - odd`",
    "   in both, rather than swapping `odd` with a second variable that nothing",
    "   ever reads, because c2mir miscompiles the swap and the MIR JIT backend",
    "   then returns a wrong number for every argument.",
    "",
    ...
    "   took under 1200.",
    "",
    "   Arguments the fraction cannot be trusted with are not answered wrongly:",
    "   they go to the asymptotic expansion below, which vouches for its own",
    "   accuracy, and are refused when that cannot either. Same rule as the C++",
    "   uses -- see issues #453 and #456. */",
    "/* Where the fraction can be trusted (issue #453). Outside this region it",
    "   converges to something that is not the ratio, with an error decay on the",
    "   way in that looks exactly like real convergence, so nothing downstream",
    "   can tell. `a` a non-positive integer terminates the fraction exactly;",
    "   `a <= 0` with `z <= 0` measured clean over 487 argument triples and is",
    "   where BNG models live; b*b >= 64*|z*a| means the odd partial numerators",
    "   are a contraction from the first step; |z| <= 20 was clean over 3400",
    "   hostile triples where the first failure anywhere is at 60. The 2*z <= b",
    "   companion for a positive z bounds the EVEN partial numerators, which the",
    "   b*b test does not reach: they carry z*(a-b-k), so for a large b they are",
    "   about z/b however small z*a is. Where that turns is sharp and it is at",
    "   z = b (clean through z/b = 1.02, 12 of 47 wrong at 1.05), so half of b",
    "   leaves a factor of two -- issue #456. Anything else is refused unless the",
    "   asymptotic route below can vouch for an answer. Kept",
    "   identical to mratio_cf_is_trustworthy in src/expression.cpp. */",
    "static int bngsim_mratio_trusted(double a, double b, double z) {",
    "    if (a <= 0.0 && a == floor(a)) {",
    "        return 1;",
    "    }",
    "    if (a <= 0.0 && z <= 0.0) {",
    ...
    "   For a large |z| the ratio has an expansion in which the Gamma factors",
    "   cancel between numerator and denominator, which is the point: the Kummer",
    "   functions themselves overflow a double long before their ratio does.",
    "",
    "     z -> -inf:  R = (b/(-z)) * 2F0(a+1, a-b+1; ; -1/z)",
    "                             / 2F0(a,   a-b+1; ; -1/z)",
    "     z -> +inf:  R = (b/a)    * 2F0(b-a, -a;    ;  1/z)",
    "                             / 2F0(b-a, 1-a;   ;  1/z)",
    "",
    "   Both series diverge, so each is summed to its smallest term and that term",
    "   estimates how close the sum came. That is what makes the route safe to",
    "   add: it certifies its own accuracy, so it is used only where the estimate",
    "   is small and the refusal is kept everywhere else. Kept identical to",
    "   mratio_asymptotic in src/expression.cpp, which carries the full note. */",
    "/* isfinite() is a <math.h> macro and the MIR JIT strips the system headers",
    "   before c2mir sees this, so the test is spelled out. It is spelled the same",
    "   way in src/expression.cpp so the two stay numerically identical. A NaN",
    "   fails it because comparisons against a NaN are false, and so does an",
    "   infinity; a magnitude past 1e308 is refused rather than classified. */",
    "static int bngsim_mratio_is_finite(double v) {",
    "    return v <= 1.0e308 && v >= -1.0e308;",
    "}",
    "",
    "static double bngsim_mratio_2f0(double alpha, double beta, double x, double *last_term) {",
    "    /* A backstop, not a tuning knob: 200 or 4000 changes nothing about",
    "       which arguments get an answer. The median series is one term long. */",
    "    const int max_terms = 500;",
    "    double t = 1.0;",
    "    double s = 1.0;",
    "    int k;",
    "    /* A zero term ends the series early, but the estimate stays the last",
    "       term ADDED. Reporting the zero would claim an accuracy the series",
    "       never demonstrated: it returned 14.33 where the ratio is 0.9998. */",
    "    for (k = 0; k < max_terms; ++k) {",
    "        const double next = t * (alpha + k) * (beta + k) * x / (k + 1.0);",
    "        if (next == 0.0 || fabs(next) >= fabs(t)) {",
    "            break;",
    "        }",
    "        s += next;",
    "        t = next;",
    ...
    "}",
    "",
    "/* Size of the branch the expansion drops relative to the one it keeps, as a",
    "   logarithm, which is what keeps it finite: the two differ by hundreds of",
    "   orders of magnitude. lgamma being +inf at the non-positive integers is",
    "   also what refuses the arguments where the branch being kept is absent",
    "   (b-a a non-positive integer for z < 0, a one for z > 0), with no separate",
    "   test for them. */",
    "static double bngsim_mratio_dropped_branch_log(double a, double b, double z) {",
    "    const double log_z = log(fabs(z));",
    "    double den;",
    "    double num;",
    "    if (z < 0.0) {",
    "        den = lgamma(b - a) - lgamma(a) + z + (2.0 * a - b) * log_z;",
    "        num = lgamma(b - a) - lgamma(a + 1.0) + z + (2.0 * a - b + 1.0) * log_z;",
    "    } else {",
    "        den = lgamma(a) - lgamma(b - a) - z + (b - 2.0 * a) * log_z;",
    "        num = lgamma(a + 1.0) - lgamma(b - a) - z + (b - 2.0 * a - 1.0) * log_z;",
    "    }",
    "    /* Written so a NaN refuses: it loses every comparison, so taking the",
    "       larger would quietly drop it, and !(x <= 0.0) is true for a NaN as",
    "       well as for anything positive. Positive is a refusal anyway -- it",
    "       says the dropped branch is the larger, which is the +inf a Gamma pole",
    "       gives. A -inf is welcome: the dropped branch is nothing. */",
    "    if (!(den <= 0.0) || !(num <= 0.0)) {",
    "        return 1.0;",
    "    }",
    ...
    "}",
    "",
    "/* Writes the ratio to *out and returns 1 when the expansion can vouch for",
    "   it, 0 when it cannot. Every comparison is written so a NaN fails it. */",
    "static int bngsim_mratio_asymptotic(double a, double b, double z, double *out) {",
    ...
    "    double err_den = 0.0;",
    "    double coeff = 0.0;",
    "    double dropped, relative, value, x;",
    "    if (z == 0.0 || !bngsim_mratio_is_finite(a) || !bngsim_mratio_is_finite(b) ||",
    "        !bngsim_mratio_is_finite(z)) {",
    "        return 0;",
    "    }",
    "    if (z < 0.0) {",
    "        x = -1.0 / z;",
    "        sum_num = bngsim_mratio_2f0(a + 1.0, a - b + 1.0, x, &err_num);",
    "        sum_den = bngsim_mratio_2f0(a, a - b + 1.0, x, &err_den);",
    "        coeff = b / (-z);",
    "    } else {",
    "        x = 1.0 / z;",
    "        sum_num = bngsim_mratio_2f0(b - a, -a, x, &err_num);",
    "        sum_den = bngsim_mratio_2f0(b - a, 1.0 - a, x, &err_den);",
    "        coeff = b / a;",
    "    }",
    "    if (sum_num == 0.0 || sum_den == 0.0 || !bngsim_mratio_is_finite(sum_num) ||",
    "        !bngsim_mratio_is_finite(sum_den)) {",
    "        return 0;",
    "    }",
    "    dropped = bngsim_mratio_dropped_branch_log(a, b, z);",
    "    if (!(dropped <= 0.0)) {",
    "        return 0;",
    "    }",
    "    relative = err_num / fabs(sum_num) + err_den / fabs(sum_den) + exp(dropped);",
    "    if (!(relative <= tol)) {",
    "        return 0;",
    "    }",
    "    value = coeff * sum_num / sum_den;",
    "    if (!bngsim_mratio_is_finite(value)) {",
    "        return 0;",
    "    }",
    "    *out = value;",
    "    return 1;",
    ...
    "    double err = 1.0 + eps;",
    "    double f = 0.0;",
    "    int odd = 1, iodd = 0, ieven = 0, j = 0;",
    "    if (!bngsim_mratio_trusted(a, b, z)) {",
    "        double asymptotic = 0.0;",
    "        if (bngsim_mratio_asymptotic(a, b, z, &asymptotic)) {",
    "            return asymptotic;",
    "        }",
    "        /* The C++ throws here. There is no exception to throw, so this is",
    "           the same NaN the iteration cap returns, and the caller turns it",
    "           into a recoverable step failure naming the time and the state. */",
    "        return NAN;",
    "    }",
    "    while (err > eps) {",
    "        double p, D, C, Delta;",
...
def _warn_functional_sens_rhs_refused(reason: str) -> None:
    ...
    **The fallback is not always correct, and the message must not say it is.**
    ...
    as a clock threshold, #381 read an equality as the surface it bounds rather
    than the measure-zero set it is, and #418 solved a clock threshold that is a
    single power of the clock (``time()*time()>=thresh``), so what still reaches
    here is the residue none of them reaches: a comparison inside a call argument,
    a comparison outside an ``if()`` head, or a clock threshold that reduces to
    neither a constant nor a single clock power over the primaries.
