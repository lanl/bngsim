...
# what none of them can bracket: cubic in the clock, so it is past both the
# single power issue #418 solves and the quadratic issue #421 solves, not affine
# for issue #48's stop time, and over no live state, so there is no residual for
# issue #150 to root on.
...
UNBRACKETED = SIR.replace(
    "    1 betaI() beta*I\n", "    1 betaI() if(time()*time()*time() + time() > 3, beta, 0)*I\n"
)
...
class TestTheGate:
    ...
    def test_a_crossing_neither_machinery_brackets_still_declines(self, tmp_path):
        """What is left after #150, #381, #418 and #421.
        ``time()*time()*time() + time() > 3`` is cubic in the clock, so it is
        neither the single power issue #418 solves nor the quadratic issue #421
        solves, it is not affine for issue #48's stop time, and it reads no live
        state, so issue #150 has no residual to root on."""
