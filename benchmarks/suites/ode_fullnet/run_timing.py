...
def time_one(model_id: str, net_file: str, model_rel: str, rn_bin: str, args) -> dict:
    ...
    if ode is None:
        ode = {"t_start": 0.0, "t_end": args.default_tend, "n_steps": args.default_nsteps,
    ...
    try:
        ...
        try:
            t, v, n, rn_timing = bc.run_network_ode(
                net_path, rn_bin,
                t_start=ode["t_start"], t_end=ode["t_end"], n_steps=int(ode["n_steps"]),
                rtol=ode["rtol"], atol=ode["atol"], out_prefix=str(workdir / "rn"),
    ...
    spec = {
        "t_start": ode["t_start"], "t_end": ode["t_end"], "n_steps": int(ode["n_steps"]),
        "rtol": ode["rtol"], "atol": ode["atol"], "n_species": n_species,
        "mode": "single_segment", "horizon_source": horizon_source,
    }
...
def main() -> int:
    ...
    ap.add_argument("--timeout", type=float, default=300.0, help="Per-engine subprocess timeout (s).")
    ...
    print("=" * 72)
    print(f"  cached networks: {sum(1 for r in nets.values() if r.get('status') == 'ok')}   to time: {len(todo)}")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(f"  [{done}/{len(todo)}] {r['outcome']:11} N={str(nsp):>5} "
...
def _write(results: dict, args) -> None:
    ...
    try:
        ...
        pass
    ordered = sorted(results.values(), key=lambda r: ((r["timing"].get("spec") or {}).get("n_species") or 0))
    from collections import Counter
    doc = {
        "_meta": {
            "suite": "ode_fullnet",
