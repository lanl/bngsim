...
emit.py
-------
Render the PSA benchmark as a Markdown report fragment.

Reads ``results/psa_results.json`` and writes
``bngsim/benchmarks/reports/generated/psa.md``.
...
DEFAULT_RESULTS = BENCH_DIR / "results" / "psa_results.json"
...
def render(payload: dict, models=None) -> str:
    """Render the S3 table.  ``models`` defaults to ``MODELS``; pass a
    filtered subsequence (see :func:`_emit.filter_by_role`) to render a
    per-audience variant."""
    if models is None:
        models = MODELS
    # PSA result rows carry both name and poplevel.
    by_key = {
        (r["name"], r.get("poplevel")): r for r in (payload.get("results", []) if payload else [])
    }
    ...
    lines = [
        ...
        r"\toprule",
        r"Model & Sp & Rxns & $N_c$ & BNGsim (s) & run\_net (s) & Speedup \\",
        r"\midrule",
    ]

    speedups: list[float] = []
    for m in models:
        for nc in m["poplevels"]:
            row = by_key.get((m["name"], nc), {})
            bn_t = _bn_time(row)
            rn_t = _rn_time(row)
            if bn_t and rn_t:
    ...
    lines.append(
        r"\multicolumn{6}{l}{\textbf{Geometric mean speedup:}} & \textbf{" + geo_cell + r"} \\"
    )
    lines += [
        r"\bottomrule",
        r"\end{tabular}",
...
def main() -> int:
    ...
    if args.dry_run:
        print(body)
        return 0
    out = write_generated("psa" + audience_suffix(args.audience), body)
